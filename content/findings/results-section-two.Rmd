...
For each Virginia county, only facilities within a specified radius would contribute to each county indicator. We calulated each index using the following formula;
...
This first map shows the level of accessibility to educational facilites for each county. Page County ranks low in accessibility to education. This result is discussed further in the isochrone mapping section. 
<center>
![Page](/findings/spatialMaps/EdAccess15.png){width=600px height=450px}
</center>

The state map below shows the level of accessibility to health care facilities for each county in Virginia. Page County ranks medium low in accessibility to health care. This result can be alternately visualized below. 
...
###Isochrone Mappings
The following travel isochrones are mapping of catchement areas calculated by method of travel and time to travel. A travel net is displayed centered at health, education and workforce training facilities to provide another measure of accessibility. 
